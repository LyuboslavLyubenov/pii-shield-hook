...
class PIIShield:
    def __init__(self, ner_model: str, threshold: float = 0.3, regex_only: bool = False):
        """
        ...
        Args:
            ...
            regex_only: If True, skip NER scanning and only use regex patterns
        """
        self.ner_model_name = ner_model
        self.threshold = threshold
        self.regex_only = regex_only
        # Initialize NER pipeline using pre-trained model (only if not regex_only)
        if not regex_only:
            ...
            for entity in ner_results:
                ...
                if entity['score'] >= self.threshold:
                    ...
                    false_positive_keywords = {
                        ...
                        # Common words in file paths that cause false positives (per requirements)
                        'url', 'user', 'users', 'home', 'path', 'file', 'dir', 'name', 'node', 'lib', 'src', 'dist', 'venv', 'config',
                        # Additional common path elements 
                        'urls', 'files', 'directory', 'folders', 'paths', 'names', 'types', 'node_modules', 'libs', 'configs', 'script', 'scripts',
                        'pipenv'
                    }
...
def main():
    ...
    if len(sys.argv) >= 3 and sys.argv[1] == "--hook-mode" and sys.argv[2] == "stdin":
        ...
        try:
            ...
            # For tool inputs, use regex_only to avoid false positives from paths
            shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=True)
            ...
            # Get regex_only mode from input data
            use_regex_only = input_data.get('regex_only', True)  # Default to True for tool arguments
            ...
            if hook_event_name == 'pretooluse':
                ...
                # For PreToolUse events, initialize shield with the desired regex_only mode
                shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=use_regex_only)
                ...
                # Handle UserPromptSubmit events - use full NER for user prompts
                shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=False)
                ...
                # For PostToolResult events, use full NER since results may contain PII in content
                shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=False)
    ...
    # Legacy hook logic (for backward compatibility)
        ...
        try:
            ...
            if tool_name == 'bash':
                # For bash commands, use full detection by default to catch secrets in command content,
                # but use regex_only when specified to prevent false positives from paths
                use_regex_only = input_data.get('regex_only', False)  # Default to False (full detection) by requirement
                command = input_data.get('command', '')
                command_shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=use_regex_only)
                redacted_cmd, mapping = command_shield.redact(str(command))
                ...
                # Use regex_only mode to avoid false positives from paths
                use_regex_only = input_data.get('regex_only', True)  # Default to True for tool args like paths
                shield_grep = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=use_regex_only)
                pattern = input_data.get('pattern', '')
                path = input_data.get('path', '')
                
                search_target = f"{pattern} {path}".strip()
                redacted_target, mapping = shield_grep.redact(search_target)
                ...
                # For read commands, use regex_only mode to prevent false positives
                # from file paths like /Users/username/ when scanning path arguments
                use_regex_only = input_data.get('regex_only', True)  # Default to True to prevent path false positives
                file_path = input_data.get('filePath', input_data.get('path', ''))
                
                # Use specified regex_only mode  
                path_shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=use_regex_only)
                redacted_path, mapping = path_shield.redact(file_path)
                
                if mapping:  # If any redaction occurred, it indicates PII/secret detection
                    sensitive_types = list(set(k.split('_')[0].strip('<>') for k in mapping.keys()))
                    error_msg = f'PII/Secrets detected in file path: {", ".join(sensitive_types)}. Blocked for security.'
                    print(error_msg, file=sys.stderr)
                    sys.exit(2)  # Exit code 2 indicates block
                ...
                # For other cases, check if regex_only was specified in input_data
                use_regex_only = input_data.get('regex_only', False)
                current_shield = PIIShield("Isotonic/deberta-v3-base_finetuned_ai4privacy_v2", regex_only=use_regex_only)
                ...
                redacted_text, mapping = current_shield.redact(scan_text)
